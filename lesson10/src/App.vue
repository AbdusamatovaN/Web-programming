<template>
    <div id="app">
        ...
        <Middle :posts="posts" :postId="postId" :users="users" :comments="comments" />
        <Footer :cntU="Object.keys(users).length" :cntP="Object.keys(posts).length"/>
    </div>
</template>
...
export default {
    ...
    beforeCreate() {
      ...
      this.$root.$on("onRegister", (login, name) => {
        if (name === "" || name > 32) {
          this.$root.$emit("onRegisterValidationError", "Name is required (0 < length < 33)");
          return;
        }
        const users = Object.values(this.users).filter(u => u.login === login);
        if (users.length !== 0 || login < 4 || login > 16 || !(new RegExp("^[a-z]+$").test(login))) {
          this.$root.$emit("onRegisterValidationError", "You have been registered or login is not from [4..16] or login does not consist only of lower latin letters");
        } else {
        ...
        this.$root.$on("onChangePostId", (id) => this.postId = id);
