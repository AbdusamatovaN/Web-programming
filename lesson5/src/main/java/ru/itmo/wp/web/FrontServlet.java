...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Method method = null;
        out: for (Class<?> clazz = pageClass; clazz != null; clazz = clazz.getSuperclass()) {
                Method[] methods = clazz.getDeclaredMethods();
                m: for (Method m : methods){
                    if (!m.getName().equals(route.action)){
                        continue;
                    }
                    Class<?>[] params = m.getParameterTypes();
                    for (Class<?> p: params){
                        if (p != Map.class && p != HttpServletRequest.class){
                              continue m;
                        }
                    }
                    method = m;
        ...
        try {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (parameterType[i] == Map.class){
                    args[i] = view;
                }
                if (parameterType[i] == HttpServletRequest.class){
                    args[i] = request;
                }
            }
        ...
        Template template = newTemplate(simpleName + ".ftlh", locale);
